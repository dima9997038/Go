public class GameOver {
  ...
  //      gameOver(copy_grid,color) ;
    ...
    public  int [] [] deleteBols(int [][] arr){
      ...
      //          input(grid);
                ...
                if (circle) {
                    Circle.delete_have_neighbor_zero(visit);
                    Circle.delete(arr,visit);
  ...
  //      System.out.println("in " + col+" "+row);
        if (col+1<5){if (visit[col+1][row]==0) dfs(col+1,row,color,arr);}
        if ((col+1)<5&&(row+1)<5){ if (visit[col+1][row+1]==0) dfs(col+1,row+1,color,arr); }
        if ((col+1)<5&&(row-1)>=0) {if (visit[col+1][row-1]==0) dfs(col+1,row-1,color,arr);}
        if (row+1<5){ if (visit[col][row+1]==0) dfs(col,row+1,color,arr);}
